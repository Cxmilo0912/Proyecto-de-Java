    ...
    <body class="bg-background-light dark:bg-background-dark font-display">
        <%
            ...
            if ("POST".equalsIgnoreCase(request.getMethod())) {
                ...
                try {
                    ...
                    if (ok) { response.sendRedirect("PControl_Finanzas.jsp"); return; }
                    else { msg = "Error al guardar el gasto"; tipo = "danger"; }
                } catch (Exception ex) {
                    msg = "Datos inválidos"; tipo = "warning";
                }
            }
        %>
        <div class="relative flex h-auto min-h-screen w-full flex-col">
            ...
            <header class="flex items-center p-4 pb-2 justify-between sticky top-0 z-10 bg-background-light dark:bg-background-dark">
                ...
                <h1 class="text-gray-900 dark:text-white text-lg font-bold leading-tight tracking-[-0.015em] flex-1 text-center">Nueva Transacción</h1>
                <div class="flex size-10 shrink-0 items-center justify-center"></div>
            </header>
            <main class="flex-1 px-4 py-2">
                <form id="gasto-form" class="w-full" method="post" action="registro_transaccion.jsp">
                ...
                <div class="flex py-3">
                    <div class="flex h-12 flex-1 items-center justify-center rounded-xl bg-gray-200 dark:bg-gray-800 p-1">
                        <label class="group flex cursor-pointer h-full grow items-center justify-center overflow-hidden rounded-lg px-2 has-[:checked]:bg-white has-[:checked]:dark:bg-black has-[:checked]:shadow-md">
                            <span class="truncate text-red-500 font-bold text-base leading-normal">Gasto</span>
                            <input checked="" class="sr-only" name="transaction_type" type="radio" value="Gasto"/>
                        </label>
                        <label class="group flex cursor-pointer h-full grow items-center justify-center overflow-hidden rounded-lg px-2 has-[:checked]:bg-white has-[:checked]:dark:bg-black has-[:checked]:shadow-md">
                            <span class="truncate text-green-500 font-bold text-base leading-normal"><a href="registroT_ingreso.jsp">Ingreso</a></span>
                            <input class="sr-only" name="transaction_type" type="radio" value="Ingreso"/>
                        </label>
                    </div>
                </div>
                <!-- Amount Input Field -->
                <div class="flex max-w-[480px] flex-wrap items-end gap-4 py-3">
                    <label class="flex flex-col min-w-40 flex-1">
                        ...
                        <input name="monto" class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl text-gray-900 dark:text-white focus:outline-0 focus:ring-2 focus:ring-primary border-none bg-gray-200 dark:bg-gray-800 h-14 placeholder:text-gray-500 dark:placeholder:text-gray-400 p-4 text-base font-normal leading-normal" placeholder="$0.00" type="number" step="0.01" value=""/>
                    </label>
                </div>
                ...
                <div class="flex max-w-[480px] flex-wrap items-end gap-4 py-3">
                    <label class="flex flex-col min-w-40 flex-1">
                        ...
                        <input name="descripcion" class="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-xl text-gray-900 dark:text-white focus:outline-0 focus:ring-2 focus:ring-primary border-none bg-gray-200 dark:bg-gray-800 h-14 placeholder:text-gray-500 dark:placeholder:text-gray-400 p-4 text-base font-normal leading-normal" placeholder="Café en la universidad" value=""/>
                    </label>
                </div>
                <!-- Details Container -->
            ...
            <footer class="p-4 pt-8 sticky bottom-0 z-10 bg-background-light dark:bg-background-dark">
                ...
                <% } %>
                <button type="submit" form="gasto-form" class="flex w-full items-center justify-center rounded-xl bg-primary h-14 text-white text-lg font-bold leading-normal shadow-lg shadow-primary/30 hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary focus:ring-offset-background-light dark:focus:ring-offset-background-dark">
                    Guardar Transacción
                </button>
            </footer>
        </div>
